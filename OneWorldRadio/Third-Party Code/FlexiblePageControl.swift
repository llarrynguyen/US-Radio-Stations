...
public class FlexiblePageControl: UIView {
    ...
    func resetIndicatorToStartPosition(){
        guard var firstItem = items.first, var lastItem = items.last else {return}
        while firstItem.index >= -2 {
            lastItem.index = firstItem.index - 1
            let lastItemFrame = CGRect(x: CGFloat(lastItem.index) * itemSize, y: 0, width: itemSize, height: itemSize)
            lastItem.frame = lastItemFrame
            items.insert(lastItem, at: 0)
            items.removeLast()
            guard let firstI = items.first, let lastI = items.last else {break}
            firstItem = firstI
            lastItem = lastI
        }
    }
    
    func resetIndicatorToEndPostion(){
        guard var firstItem = items.first, var lastItem = items.last else {return}
        while firstItem.index < 0 {
            firstItem.index = lastItem.index + 1
            let firstItemFrame = CGRect(x: CGFloat(firstItem.index) * itemSize, y: 0, width: itemSize, height: itemSize)
            firstItem.frame = firstItemFrame
